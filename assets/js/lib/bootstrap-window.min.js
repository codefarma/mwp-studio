...
(function($) {
    ...
    Window.prototype.initialize = function(options) {
		...
		if (options.minimizable && !options.modal) {
            options.elements.buttons.minimize = $('<button title="Minimize" data-minimize="window"><i class="glyphicon glyphicon-chevron-down"></i></button>');
            options.elements.handle.prepend(options.elements.buttons.minimize);		
		}
		
        if (options.maximizable) {
            options.elements.buttons.maximize = $('<button title="Maximize" data-maximize="window"><i class="glyphicon glyphicon-chevron-up"></i></button>');
            options.elements.handle.prepend(options.elements.buttons.maximize);
            options.elements.buttons.restore = $('<button title="Restore" data-restore="window"><i class="glyphicon glyphicon-modal-window"></i></button>');
            options.elements.handle.prepend(options.elements.buttons.restore);
        }
		
        if (_this.$el.find('[data-dismiss=window]').length <= 0) {
            options.elements.handle.prepend('<button title="Close" type="button" class="close" data-dismiss="window" aria-hidden="true"><i class="glyphicon glyphicon-remove"></i></button>');
        }
		...
		options.elements.body.css('margin-bottom',options.elements.footer.outerHeight());
    ...
    Window.prototype.restore = function() {
        if ( ! this.$el.hasClass('minimized') && ! this.$el.hasClass('maximized') ) {
			return;
		}
		this.$el.removeClass('minimized');
        this.$el.removeClass('maximized');
		...
		var beforeMinimized = this.$el.beforeMinimized.pop();
		
		if(beforeMinimized) {
            this.$el.css(beforeMinimized);
        } else {
            this.$el.css({
	...
	Window.prototype.sizeBody = function( forceHeight ) {
		var height = this.options.elements.footer.offset().top - ( this.options.elements.body.offset().top  );        
		if ( forceHeight ) {
			this.options.elements.body.css({ height: height, minHeight: 'auto' });
		} else {
			this.options.elements.body.css({ minHeight: height, height: 'auto' });
		}
	};
    ...
    Window.prototype.submit = function() {
        var _this = this;
		if ( typeof _this.options.submit === 'function' ) 
		{
			var deferredSubmit = $.when( _this.options.submit( _this ) ).done( function( result ) {
				if ( result !== false ) {
					_this.close();
				}
			});
			_this.$el.trigger(namespace + '.submit', deferredSubmit);
		} else {
			_this.close();
		}
    };
    ...
    Window.prototype.initHandlers = function() {
        ...
        this.$el.find('[data-submit=window]').on('click', function(event) {
            event.stopPropagation();
            event.preventDefault();
            if (_this.options.blocker) {
                return;
...
(function($) {
    ...
    WindowManager.prototype.resortWindows = function() {
        var startZIndex = 900;
        $.each(this.windows, function(index, window) {
			var addon = window.options.modal ? 100 : 0;
            window.setIndex(startZIndex + index + addon);
        });
    };
    ...
    WindowManager.prototype.addWindow = function(window_object) {
        ...
        window_object.on('bsw.close', function() {
        ...
        if (this.options.container && !window_object.options.modal) {
		...
		if ( window_object.options.modal ) {
			Window.openModals += 1;
			if ( ! $('#window-modal-backdrop').length ) {
				$('body').append('<div id="window-modal-backdrop" class="fade in"></div>');
			}
			window_object.$el.on( 'bsw.close', function() {
				Window.openModals -= 1;
				if ( Window.openModals < 1 ) {
